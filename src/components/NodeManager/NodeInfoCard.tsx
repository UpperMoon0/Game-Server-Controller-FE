            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full ${
                ...
                <div className={`w-2 h-2 rounded-full ${
                  node.status === 'running' ? 'bg-neon-green animate-pulse' : 
                  node.status ? 'bg-red-500' : 'bg-gray-500'
                }`} />
                <span className={`font-medium ${
                  node.status === 'running' ? 'text-neon-green' : 
                  node.status ? 'text-red-400' : 'text-gray-400'
                }`}>
                  {node.status || 'N/A'}
                </span>
              </div>
